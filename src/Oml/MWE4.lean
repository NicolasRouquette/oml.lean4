...
def State.wff: State → Bool :=
  fun s => s.declarations.toList.all fun (_, xs) => xs.toList.all s.declarations.contains
...
theorem State.withDeclarationWff (s: State) (d: String) : s.wff = true → (s.withDeclaration d).wff = true := by
  intro h
  sorry

theorem State.withSpecializationWff (s: State) (sub: String) (sup: String): s.wff = true → (s.withSpecialization sub sup).wff = true := by
  intro h
  sorry
